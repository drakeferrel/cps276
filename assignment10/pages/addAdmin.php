...
function addData($post, $postArr){
  /* IF EVERYTHING WORKS ADD THE DATA HERE TO THE DATABASE HERE USING THE $_POST SUPER GLOBAL ARRAY */
	  ...
	  // Check if this email is already assigned to an admin
	  $emailHasAccount = false;
	  $emailHasAccountSql = "SELECT email_address FROM admins";
	  
	  $records = $pdo->selectNotBinded($emailHasAccountSql);
	  foreach($records as $row) {
		  if($post['email'] == $row['email_address']) $emailHasAccount = true;
	  }

	  if ($emailHasAccount == false) {
		  $sql = "INSERT INTO admins (name, email_address, password, status) VALUES (:name, :email_address, :password, :status)";
		  ...
		  $result = $pdo->otherBinded($sql, $bindings);
			...
			return getForm("<p>This email is already being used for an account</p>",$postArr);
	  }
      ...
      if($result == "error"){
        ...
        return getForm("<p>Admin Added</p>", $postArr);
      }
...
return [$acknowledgement, $form, "<h1>Add Admin</h1>"];
