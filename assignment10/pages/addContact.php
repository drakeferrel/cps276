...
$elementsArr = [
  ...
  "state"=>[
    "type"=>"select",
    "options"=>["mi"=>"Michigan","oh"=>"Ohio","pa"=>"Pennslyvania","tx"=>"Texas","va"=>"Virginia"],
		"selected"=>"oh",
		"regex"=>"name"
	],
  "email"=>[
 ...
 "dob"=>[
	  "errorMessage"=>"<span style='color: red; margin-left: 15px;'>Date of birth cannot be blank, must be a valid date and be formatted as mm/dd/yyyy</span>",
    "errorOutput"=>"",
    "type"=>"text",
    "value"=>"12/25/1999",
		"regex"=>"date"
...
function addData($post, $postArr){
  ...
  /* IF EVERYTHING WORKS ADD THE DATA HERE TO THE DATABASE HERE USING THE $_POST SUPER GLOBAL ARRAY */
      ...
      $sql = "INSERT INTO contacts (name, address, city, state, phone, email_address, dob, newsletter, email_updates, text_updates, age_range) VALUES (:name, :address, :city, :state, :phone, :email_address, :dob, :newsletter, :email_updates, :text_updates, :age_range)";
	  ...
	  $bindings = [
		...
		[':email_address',$post['email'],'str'],
		[':dob',$post['dob'],'str'],
		[':newsletter',$newsletter,'int'],
		[':email_updates',$email_updates,'int'],
		[':text_updates',$text_updates,'int'],
		[':age_range',$post['ageRange'],'str']
	  ];

      $result = $pdo->otherBinded($sql, $bindings);

      if($result == "error"){
        return getForm("<p>There was a problem processing your form</p>", $postArr);
      }
      else {
        return getForm("<p>Contact Information Added</p>", $postArr);
      }
...
$form = <<<HTML
	...
	<div class="form-group">
      <label for="name">Address (just number and street){$elementsArr['address']['errorOutput']}</label>
      <input type="text" class="form-control" id="address" name="address" value="{$elementsArr['address']['value']}" >
    </div>
	
	<div class="form-group">
      <label for="name">City{$elementsArr['city']['errorOutput']}</label>
    ...
    <div class="form-check form-check-inline">
      <input class="form-check-input" type="checkbox" name="contact[]" id="contact2" value="emailupdates" {$elementsArr['contact']['status']['emailupdates']}>
...
return [$acknowledgement, $form, "<h1>Add Contact</h1>"];
